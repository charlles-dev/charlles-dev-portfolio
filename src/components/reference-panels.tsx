    ...
    <li className="works-rise works-entry">
      <article className="works-project">
        <div className={`works-project-media works-project-media-${index + 1}`}>
          ...
          <i aria-hidden="true" />
          <button type="button" className="works-media-button" aria-label={`${project.name} visual`}>View</button>
...
function WorkPanel({ dictionary, onClose }: { dictionary: PortfolioDictionary; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <section className={`works-panel bg-card fixed inset-x-0 top-3 bottom-0 z-100 flex flex-col overflow-hidden rounded-t-3xl outline-none ${themeMode === "light" ? "is-light" : ""}`} data-state="open" data-panel-theme={themeMode} role="dialog" aria-modal="true" aria-label={dictionary.work.panelTitle} tabIndex={-1}>
        <header className="works-rise border-border/70 flex shrink-0 items-center gap-2 border-b px-4 py-3 sm:gap-4 sm:px-8 sm:py-4">
          ...
          <div className="reference-theme-switch" role="group" aria-label="Tema">
            <button type="button" aria-pressed={themeMode === "light"} className={themeMode === "light" ? "is-active" : ""} onClick={() => setThemeMode("light")}><ThemeGlyph mode="light" />Claro</button>
            <button type="button" aria-pressed={themeMode === "dark"} className={themeMode === "dark" ? "is-active" : ""} onClick={() => setThemeMode("dark")}><ThemeGlyph mode="dark" />Escuro</button>
          </div>
