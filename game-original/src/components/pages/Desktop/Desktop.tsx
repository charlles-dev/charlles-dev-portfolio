...
export const Desktop: React.FC<DesktopProps> = ({ apps = [], profile = 'guest' }) => {
  ...
  useEffect(() => {
    ...
    if (profile === 'admin') {
      const adminApp = apps.find(a => a.id === APP_IDS.SERVER_CONSOLE);
      if (!adminApp) return;
      const timer = window.setTimeout(() => openWindow(adminApp), 350);
  ...
  return (
    <main className={`desktop desktop--${profile}`}>
