...
export const useGameLogic = () => {
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if ((e.key === 'e' || e.key === 'E' || e.key === 'Enter') && nearbyObject) {
                e.preventDefault();
                triggerObjectEvent(nearbyObject.id);
                return;
            }

            if (gameEventMessage) return;
    ...
    }, [movePlayer, gameStatus, setFacing, nearbyObject, triggerObjectEvent, gameEventMessage]);
