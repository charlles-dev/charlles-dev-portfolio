...
export const useGameInteraction = (
    ...
    const nearbyObject = useMemo(() => {
        for (const obj of WORLD_ENTITIES.filter((entity) => entity.isInteractive !== false)) {
            const playerCenter = { x: player.position.x + TILE_WIDTH / 2, y: player.position.y + TILE_HEIGHT / 2 };
            const nearestX = Math.max(obj.position.x, Math.min(playerCenter.x, obj.position.x + obj.size.x));
            const nearestY = Math.max(obj.position.y, Math.min(playerCenter.y, obj.position.y + obj.size.y));
            const dist = Math.hypot(playerCenter.x - nearestX, playerCenter.y - nearestY);
            const avgTileSize = (TILE_WIDTH + TILE_HEIGHT) / 2;
            if (dist <= avgTileSize * 1.25) {
                return obj;
            }
        }
        return null;
