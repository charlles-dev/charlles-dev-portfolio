...
import './App.css';
import type { SessionProfile } from '@/data/sessionProfiles';
...
const App: React.FC = () => {
  ...
  const [sessionProfile, setSessionProfile] = useState<SessionProfile>('guest');
  const sessionApps = useMemo(() => AppRegistry.map(app => {
    if (app.id !== 'server_console') return app;
    return { ...app, visibility: sessionProfile === 'admin' ? { desktop: true, programs: true } : { desktop: false, programs: false } };
  }), [sessionProfile]);
  ...
  const handleLogin = (profile: SessionProfile) => {
                  ...
                  >
                    ...
                    <main>
                      <Suspense fallback={<div className="app-loader"><LoadingSpinner /></div>}>
                        ...
                        {systemState === 'desktop' && <Desktop apps={sessionApps} profile={sessionProfile} />}
