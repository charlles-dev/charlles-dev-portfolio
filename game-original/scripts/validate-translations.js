...
function extractKeysFromTS(filePath) {
    ...
    const declaration = content.match(/export type TranslationKeys\s*=([\s\S]*?);/);
    if (!declaration) throw new Error('TranslationKeys declaration not found');
    const keys = [];
    const regex = /\|\s*'([\w_]+)'/g;
    let match;
    while ((match = regex.exec(declaration[1])) !== null) {
        keys.push(match[1]);
    }
    return keys;
}
